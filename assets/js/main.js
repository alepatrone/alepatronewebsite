...
(function ($) {
  ...
  if ($(tpSideMenu).find(".tp-submenu, .mega-menu").length != 0) {
    $(tpSideMenu)
      ...
      .parent()
      .append(
        '<button class="tp-menu-close"><i class="fa-solid fa-angle-right"></i></button>'
      );
  }
  var sideMenuList = $(
    ".tp-offcanvas-menu nav > ul > li button.tp-menu-close, .tp-offcanvas-menu nav > ul li.has-dropdown:has(ul) > a, .tp-offcanvas-menu nav > ul li.has-dropdown > ul > li.menu-item-has-children > a"
  );
    ...
    function updateTransform() {
      if (window.innerWidth < 1024) {
        ...
        $projects.each(function () {
          ...
          $inner.css({
            transform: `rotateX(${rotateX.toFixed(
              3
  ...
  if (document.querySelectorAll(".bf-portfolio-vp-rotate-wrap")) {
    mm.add("(min-width: 1200px)", () => {
      document
        .querySelectorAll(".bf-portfolio-vp-rotate-wrap")
        .forEach((item) => {
          const leftItem = item.querySelector(".bf-portfolio-vp-rotate-item-1");
          const rightItem = item.querySelector(
            ".bf-portfolio-vp-rotate-item-2"
          );
          gsap.set(leftItem, {
            x: -300,
            y: 200,
  ...
  if (document.querySelector(".bf-portfolio-3-item")) {
    ...
    pw.add("(min-width: 991px)", () => {
      ...
      document.querySelectorAll(".bf-portfolio-3-item").forEach((item) => {
        ...
        tl.to(item.querySelector(".bf-portfolio-3-thumb.item-1"), {
          x: 0,
          rotate: 0,
        }).to(
          item.querySelector(".bf-portfolio-3-thumb.item-2"),
          { x: 0, rotate: 0 },
          0
        );
      });
